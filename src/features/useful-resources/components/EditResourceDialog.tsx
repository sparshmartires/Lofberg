...
import type { UsefulResourceItem } from "./UsefulResourcesTable"

interface EditResourceDialogProps {
  open: boolean
  onOpenChange: (open: boolean) => void
  resource: UsefulResourceItem | null
}
...
export default function EditResourceDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        <div className="mb-6">
          <h2 className="text-[20px] text-[#1F1F1F]">Edit resource</h2>
          <p className="text-[14px] text-[#747474]">Update resource details below.</p>
        </div>

        <div className="space-y-6">
          <div>
            <p className="text-sm mb-2">Resource title*</p>
            <Input
              value={title}
